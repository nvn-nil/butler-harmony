            ...
            if file.endswith(".yaml") or file.endswith(".yml"):
                ...
                with open(filepath) as fi:
                    ...
                    if id not in self._configs:
                        ...
                        raise Exception(f"Tools cannot have duplicate id: {id}]\n\t Tool {self._configs[id]['name']} already has the same id")

    def register_tool(self, id, name, description, current_version, location, name_format, last_updated, type, update_location, update_frequency, archive_location):
        ...
        tool_config = {
            ...
            'last_updated': last_updated.strftime("%Y-%m-%d %H:%M") if last_updated else datetime.now().strftime("%Y-%m-%d %H:%M"),
        }
